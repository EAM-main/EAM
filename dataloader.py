...
class EAM_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #  title&&transcript and event(LLM response)
...
def collate_fn_EAM(batch):
    ...
    return {
        'vid': vid,#video_id
        'label': label,#real/fake classification label
        'fps': fps,
        'total_frame': total_frame,
        'text_fea': text_fea, #title&transcript text features
        'visual_fea': visual_fea, #visual_frame features
        'audio_fea': audio_fea,#audio features
        'event_fea': event_fea,#event text features(LLM response)
        'event_ord': event_ord #event text classification label(used in memory banl)
    }
